...
@testset "Tests for TMNAbsRem" begin
    ...
    @testset "TMNAbsRem constructors" begin
        xm = TMNAbsRem{2, Interval{Float64}, Float64}(x, zi, b0, ib0)
        ym = TMNAbsRem{2, Interval{Float64}, Float64}(y, zi, b0, ib0)
        @test xm == TMNAbsRem(x, zi, b0, ib0)
        @test ym == TMNAbsRem(y, zi, b0, ib0)
        ...
        # Test errors in construction
        @test_throws AssertionError TMNAbsRem(x, zi, IntervalBox(1..1), IntervalBox(1..1))
        @test_throws AssertionError TMNAbsRem(x, zi, b0, ib1)
        @test_throws AssertionError TMNAbsRem(x, 1..1, b0, ib0)
    ...
    #         TMAbsRem1(interval(5.0), 4, x0, ii0)
    #     @test rpa(x->5*x, TMAbsRem1(4, x1, ii1)) ==
    #         TMAbsRem1(5.0*(x1+Taylor1(4)), x0, x1, ii1)
    #     @test rpa(x->5*x^4, TMAbsRem1(4, x0, ii0)) ==
    #         TMAbsRem1( interval(5.0)*Taylor1(4)^4, x0, x0, ii0)
    #     @test rpa(x->5*x^4, TMAbsRem1(3, x0, ii0)) ==
    #         TMAbsRem1( Taylor1(x0, 3), 5*ii0^4, x0, ii0)
    #
    #     # Testing remainders of an RPA
    #     order = 2
    #     ii = ii0
    #     xx = x0
    #     ftest = x -> exp(x)-1
    #     tma = rpa(ftest, TMAbsRem1(order, xx, ii))
    #     tmb = ftest(TMAbsRem1(order, xx, ii))
    #     @test tma == tmb
    #     fT, Δ, ξ0 = rpafp(tma)
    #     @test interval(ftest(ii.lo)-fT(ii.lo-ξ0),
    #                     ftest(ii.hi)-fT(ii.hi-ξ0)) ⊆ remainder(tma)
